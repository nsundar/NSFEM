...
c  
        ...
        CC(2,1)=F(1,2)*F(1,1)+F(2,1)*F(2,1)
        ...
        DO ii=1,2
	  	DO jj=1,2
        	if (ii .EQ. jj)  then
	   		pb(ii,jj)=lam*lndetF*INVCC(ii,jj)-mu*(INVCC(ii,jj)-1)
        	else
            pb(ii,jj)=lam*lndetF*INVCC(ii,jj)-mu*INVCC(ii,jj)
          	endif
	  	ENDDO
        ENDDO
c  
	  CMAT(1,1)=(-2*lam*lndetF+lam+2*mu)*INVCC(1,1)*INVCC(1,1)
	  CMAT(1,2)=(-2*lam*lndetF+2*mu)*INVCC(1,2)*INVCC(1,2)+
     1lam*INVCC(1,1)*INVCC(2,2)                                                          
	  CMAT(1,3)=(-2*lam*lndetF+lam+2*mu)*INVCC(1,2)*INVCC(1,1)
	  CMAT(2,1)=CMAT(1,2)
      CMAT(2,2)=(-2*lam*lndetF+lam+2*mu)*INVCC(2,2)*INVCC(2,2)
	  CMAT(2,3)=(-2*lam*lndetF+lam+2*mu)*INVCC(1,2)*INVCC(2,2)  
      CMAT(3,1)=CMAT(1,3)
      CMAT(3,2)=CMAT(2,3)
	  CMAT(3,3)=(-lam*lndetF+lam+mu)*INVCC(1,2)*INVCC(1,2)+
     1(-lam*lndetF+mu)*INVCC(1,1)*INVCC(2,2)
c        
		S(1,1)=pb(1,1)
 		S(1,2)=pb(1,2)
        S(2,1)=pb(2,1)
